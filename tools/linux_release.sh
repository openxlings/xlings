...
# same job.
BIN_SRC="$(find "$PROJECT_DIR/target/$MCPP_TARGET" -path '*/bin/xlings' -type f -perm -111 -exec ls -t {} + | head -1)"
...
# Shipped example themes, copied as DATA.
#
# They used to be C++ raw string literals in
# `src/core/xself/theme_resources.cppm`, written to disk by `self init` during
# packaging and read back at runtime -- a round trip through source code for
# content that is a config file at both ends. `default` stays compiled in
# (theme::builtin_default), because "the config directory is empty" must still
# have colours; these two are examples of the format users copy and edit, and
# an example must be a file.
if [[ -d config/themes ]]; then
  mkdir -p "$OUT_DIR/config/themes"
  cp config/themes/*.json "$OUT_DIR/config/themes/"
else
  fail "config/themes missing from the source tree"
fi
...
# Assert the themes are IN the artifact.
#
# There was no check at all while they were generated by `self init`, so
# "the tarball carries the themes" was an assumption nobody tested -- and
# openxlings/xim-pkgindex#672 was filed because a different link in the same
# chain silently dropped them. Now they are a plain copy, and a plain copy is
# exactly the kind of step that fails by producing fewer files rather than an
# error.
for t in mono high-contrast; do
  [[ -f "$OUT_DIR/config/themes/$t.json" ]] \
    || fail "config/themes/$t.json missing from the package"
done
python3 -c "import json,sys; [json.load(open(p)) for p in sys.argv[1:]]" \
  "$OUT_DIR/config/themes/mono.json" "$OUT_DIR/config/themes/high-contrast.json" \
  || fail "a shipped theme is not valid JSON"
