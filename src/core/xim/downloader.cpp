...
module xlings.core.xim.downloader;
...
bool commit_staging_file_(const std::filesystem::path& staging, const std::filesystem::path& destination, std::string& error, bool failAfterBackupForTest) {
...
DownloadResult download_one(const DownloadTask& task, std::function<void(double total, double now)> onProgress, CancellationToken* cancel, const DownloadTestHooks_* testHooks) {
    namespace fs = std::filesystem;
...
download_all(std::span<const DownloadTask> tasks, const DownloaderConfig& config, DownloadProgressRenderer onRender, std::function<void(std::string_view name, float progress)> onProgress, CancellationToken* cancel) {
