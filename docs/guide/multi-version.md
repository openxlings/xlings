...
看着绕，但它挡掉的是一个更难查的结果："直接激活" 只能激活 gcc 这个目标，
不会补上它依赖的 glibc —— 你会得到一个能跑、`-print-sysroot` 也正确、唯独
编译不了的工具链，而且全程没有任何提示。依赖边由解析器记录（2026.9.26.1 起），
`install` 负责解析依赖、写入 `installed[]` 并安装依赖闭包；`use` 只在已有
`installed[]` 之间切换，不重做这一步。
