...
但把这件事交给 `xlings use` 是错的，`2026.7.31.3` 已经改掉。`use` 只在**本 SubOS 的 `installed[]` 里**切换；某个版本在别的 SubOS 装过，不代表它在这里能用。**依赖边现在是解析结果的一部分**（2026.9.26.1 起，`DepEdge` 记录 `name`、版本约束、依赖种类，以及该名字应当在哪个声明索引里解析），但 `use` 不会重新做解析：它只切换已经纳入当前 SubOS 的目标。直接激活一个没有依赖闭包的版本会得到能跑、`-print-sysroot` 也对、唯独编译不了的工具链。纳入新 SubOS 由 `xlings install` 负责 —— 它解析依赖并写下依赖边，移除保护会读取同一条边；因为 payload 共享，已装过的部分同样不会重新下载。
