...
**谁决定一个键有没有前缀，以及什么时候决定。** 这是契约的一部分（2026.9.2.1 起）：

- 来自**默认索引**（名叫 `xim` 的那条 `index_repos` 条目）的包写**裸键**，
  其它索引写 `ns:` 前缀。判定看条目的**名字**，永远不看它在数组里的**位置**——
  `index_repos` 的条目是平级的，`xlings index use` / `config --index-repo` 改变
  排列不会改变任何包的键。
- 一个键的拼法在**第一次写入时决定，之后不再重新推导**。同一个 provider 再注册同一
  版本（重装、修复）时沿用磁盘上已有记录的拼法；只有从未注册过的版本才按当前判定铸键。
- 读侧对两种拼法**双向宽容**：`xim:2.1.222` 能找到存成 `2.1.222` 的记录，反之亦然。
  但 `ns:v` 形式的查询**不会**命中另一个命名空间下的 `other:v`——那是另一个索引的
  payload。
- 同一版本若同时存在裸键和带前缀键、且 `path` 相同，那是**同一次注册被写了两遍**
  （历史上判定随数组位置翻转造成的损伤）。所有读者把这一对当作一条记录，以带
  `bindingGroup` 的那条为准；`xlings self doctor` 报告为 `duplicate version key`，
  `--fix` 合并它们并改写所有 subos 里指向被合并键的引用。
...
`--fix` 会修复不一致的部分 —— 详见[自我管理与修复](../guide/self-management.md)。
...
**`latest` 与「表里的最大项」是对同一张表问的两个不同问题**，只在没人压住版本时
...
对 hosted runtime 而言，「声明的 runtime 必须被装上」是个**假命题**——
