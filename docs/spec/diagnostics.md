...
`Config::version_origin(target)` 给出这个答案;新增任何"由配置决定"的失败都应该填它。

它返回**两半**:`source`(展示用的文件+字段)和 `fromProjectManifest`(哪一层赢了)。合成一个返回值不是为了省事 —— 是因为把这两个问题分开问,调用方就会用代理去重建后一半,而代理会错(见 2.1b)。

### 2.1b 动作要指向**用户该跑的那条**,不是等价的手工版

项目里钉住的版本没装时,动作是 `xlings install`(无参数,读项目声明),**不是** `xlings install <name>@<ver>`。后者让用户重做一遍项目文件里已经写好的事,而且项目声明多个依赖时它还是错的。

同理:`facts` 里的候选列表在这种场景只留最新 2 个 —— 读者是要去跑那条动作的,不是来挑版本的,长列表只会把动作挤出屏幕。

**但"该跑哪条"取决于是哪一层钉的,不是"在不在项目里"。** `xlings install`(无参)只读向上找到的 `.xlings.json` 的 `workspace`;全局 subos 文件和 project-subos 文件同样会钉版本、同样会渲染出 `from` 行,而这条命令都不读。所以判据必须是 `version_origin().fromProjectManifest`,用 `has_project_config()` 会在"目录里恰好有个项目文件、但这个包是全局钉的"时同时说错三件事:项目并没有要求它、`from` 指的不是项目清单、给出的命令会装别的东西然后退出 0。E2E-61 的 S2c 专门钉这个。
...
| 级别 | 用于 | 退出码 |
|---|---|---|
| `Error` | 命令失败了,而且 xlings 解决不了 | 非 0 |
| `Warn` | 降级了继续跑;**或者**失败了但只差一条已知命令 | 通常 0,见下 |
| `Note` | 关于一件**成功**的事的说明 | 0 |

**严重级说"这有多糟",退出码说"发生了什么" —— 两者可以不一致。**

具体地:项目 `.xlings.json` 声明了一个还没装的版本时,shim 报 `Warn` 而**退出码仍是 1**。理由:

- 命令确实失败了(程序没跑起来),脚本必须看得见 → 退出码非 0;
- 但这是一个**还没配置好的项目**,离能用只差 `xlings install`,而且 xlings 知道是哪条命令 → 红色加粗的 `[error]` 会把"你的项目需要初始化"读成"工具坏了"。

判据是**谁能解决**:xlings 知道确切出路的,用 `Warn`;需要用户去查、去想、去别处解决的,用 `Error`。
