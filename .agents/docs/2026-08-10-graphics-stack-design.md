...
| 项 | 判据 |
|---|---|
| E1a | 重装任一 GL 应用后,`readelf -d` 显示 `(RPATH)`;`matrix.sh` 标签差分行两侧一致 |
| E1b | 故意把一个包的产物改回 RUNPATH,装机时被报出;显式退出的包不被报 |
| E2 | 用户零 flag:`gcc -lGL` 链接+运行 OK;新建程序四个入口点到 GPU |
| E3a | `subos info` 在驱动漂移时说话;不漂移时不打扰 |
| E3b | `--sandbox` 无 GPU 时提示 `--gpu`;有 `--gpu` 时不提示 |
