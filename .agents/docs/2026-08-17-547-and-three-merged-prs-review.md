...
落地时具体看了那两个站点，结论是它们**需要的不是同一个函数**：

| 站点 | 障碍 |
|---|---|
| `subos.cpp:1259` `subos remove` | `payload_trash_root()` 是从 `xpkgs` 祖先推出来的（为了 `rename` 不跨文件系统）。一个 subos 目录没有这个祖先，所以复用它只会走到「跳过挪移」。要真正复用，得把 trash 根一般化成一个参数 —— 那是 API 改动，不是替换一行 |
| `xself/uninstall.cpp:302` `self uninstall` | 它删的就是 home 本身。**trash 放哪里都会跟着被删** —— 「挪走再删」这套策略在这里没有落脚点 |
| `subos.cpp:531` | 是 `create` 失败后的回滚，删的是刚建好的目录，几乎不可能被占用 |

另外 `subos remove` 今天失败时**是报错的**（`ErrorCode::Permission`，且保留注册项），
不是静默 —— 所以它属于「Windows 上更难用」，不属于「说了假话」。

**能便宜复用的只有清只读位那一步**，但它现在在 `payload.cpp` 的匿名 namespace 里。
把它导出 + 用在两个没有测试覆盖的站点上，是发布前不该做的改动。

**记在这里，是因为「没做」和「做不了」在清单里长得一样。**
