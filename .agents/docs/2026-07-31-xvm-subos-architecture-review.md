...
C 是**直接病因**，B 让它不可能被彻底修对，A 让人无法判断修得对不对。
...
### D5. 把 sandbox 从 subos 里拆出来（消 A）

`subos.cppm` 2285 行里，proot/bwrap 后端、storage image、GPU 与"版本选择 + FHS 视图"没有共享的不变量。拆成 `subos`（选择域 + 视图）与 `sandbox`（运行时隔离），后者以前者为输入。

收益主要是**词汇上的**：拆开之后，"这个状态该不该 per-subos"才有唯一答案。这条不改变任何行为，但它是让 A 类讨论收敛的前提。
