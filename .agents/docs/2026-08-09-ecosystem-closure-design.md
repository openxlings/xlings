...
| # | 执行点 | 看得见 | 判得了 | 今天 |
|---|---|---|---|---|
| 1 | **索引 CI**(`posix-test.sh`) | recipe + 刚装的载荷 | D1 + D2 | ✔ 已有 |
| 2 | **客户端安装期**(`xlings install`) | recipe(刚跑完) + 载荷 | D1 + D2 | ✘ 缺 |
| 3 | **用户构建期**(`mcpp build/run`) | 产物 ELF + 已装载荷 | **只有 D2** | ✘ 缺 |

执行点 2 值得单独说:**客户端此刻手里是有 recipe 的**(它刚执行完 install/config),
所以它能判的和索引 CI 一样多。今天它一条都不判——同一份逻辑,在 CI 里是硬失败,
在用户机器上完全不存在。`mcpp#392` 的用户装 llvm 时没有任何东西拦过他。

执行点 3 是 `mcpp#392` 的正位。用户产物**没有 recipe**,所以:

> **用户产物上的「规则 D」= D2,不是 D1。**

这不是妥协,而是恰好对:D1 问「你声明得对不对」,用户没有要声明的东西;
D2 问「这个二进制跑不跑得起来」,而那正是要拦的。
...
规则 D 一旦对用户产物生效,`mcpp#392` 那位用户的项目会**立刻构建失败**,
而不是像现在跑起来才炸。方向对——失败提前到了能修的地方——但顺序不能反:

```
1. 先补已知缺口(ncurses)               ← 否则守卫开了就是拦自己人
2. 执行点 2(安装期)先上,warn-only     ← 收集真实缺口清单
3. 缺口清完,再把执行点 2/3 转成硬失败
```

**先开守卫再补包 = 把一个运行时故障换成一个构建期故障,用户体感更差。**
