...
- ~~**D5 的索引侧**~~ —— **已做**,见 openxlings/xim-pkgindex#689,且
  **§6 里「c 不能早于 a 到达用户」的排期被实测推翻**(第七条之外的第八条):
  索引侧自己带了 `sysroot.unwrap_directory_asset`,在 Lua 里做和客户端同款的
  **无损解包**,所以**老客户端 2026.8.22.4 装新索引同样安全**——实测两个安装
  顺序、迁移路径、老客户端四种情形下 `usr/include/scsi` 都是 9 项真目录,
  两个 payload 都没被写过。

  过程中还推翻了我自己刚写的一版:那一版的 `unwrap_directory_asset` 只是
  `rm` 掉陈旧链接,实测在**新客户端**上把对方 6 个头文件扔了——而客户端本来
  会留着。**一个让新客户端表现更差的迁移辅助不叫辅助。**
