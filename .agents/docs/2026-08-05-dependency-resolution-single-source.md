...
```
INTERP 所属的 payload 目录  ==  RPATH 中该 provider 的 payload 目录
```

不等就**中止安装**,并打印两个具体路径和各自的来源。这条断言的成本是一次字符串比较,
而它拦下的是本文第 2 节那个错误 —— 后者要用 `LD_DEBUG` 才能定位。

**这条断言已经先做成可执行判据并实测过**,见
`.agents/tools/check-loader-libc-same-source.sh`。它不需要等实现落地:

```
构造出的分叉包           FAIL   INTERP → glibc/2.39 , RPATH → glibc/2.44
正常包                   OK
三个已装生态全量扫描      196 个二进制,OK=196  FAIL=0  误报 0
```

零误报是它能直接升级为 doctor 规则的前提 —— 一条会误报的检查,
用不了几次就会被当成噪音跳过,那和没有是一样的。
