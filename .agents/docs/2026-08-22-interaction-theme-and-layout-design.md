...
## 7. 实施记录(2026.8.22.3 / PR #558)

六项裁决(D-a…D-f)全部按建议采纳并落地,外加用户在实施期间追加的三条要求(§4b.5 标签、§4b.6 帮助文本、§4b.7 一语言一文件)与 `src/libs` 并入 `modules`。

**落地**:T1 T2 T3 T4 T5 T6 T8 T9 T11 T12 T14 T15 T16 T17 T18 T19

**未做,及原因**:

| | 状态 | 为什么 |
|---|---|---|
| T7 | 未做 | 选择层纳入 `subos use` / `--theme` / shim 三处;本轮已改动面过大,留作下一轮 |
| T10 | **无需改动** | 索引配方 `__install_home_config` 本来就是从 payload 文件拷贝的,T9 之后它成为唯一机制,recipe 一个字都不用改 |
| T13 | 部分 | i18n 机制与目录已就位,覆盖了帮助与 `config` 标签;`diag` 的 15 个 code 尚未接入 |
| T20 | 未做 | `config --lang/--mirror` 不带值时进入选择,依赖 T7 的选择层 |

**实施中被推翻或修正的三处**:

1. **§4b.3 的去依赖化不再必要。** 原计划让 `i18n` 成为叶子,为此要删掉 `language()` 的惰性回退。`platform` 也独立成 module 后,成员之间可依赖(mcpp 文档 §2.3 明确写着),`i18n` 保持原样搬家即可。
2. **`src/libs` 并入 `modules`。** 原方案写"它们是第三方单元,留在原地";用户指出这个项目构建并导出的模块就是这个项目的模块。`libs/` 与 `src/libs/` 两个名字指同一件事,本身就是要消除的歧义。
3. **T10 是空操作。** 方案把它列为跨仓库任务并加了顺序约束;实际检查发现配方读的就是 payload 文件,不涉及内嵌。顺序约束仍然成立(payload 不存在时配方是空操作),只是这次不必动它。

**实施中发现的、方案没有预见的三个问题**:

1. **`xlings config` 有两个渲染实现。** `cli.cpp` 自建了一份 `xself::cmd_config` 的近似副本,而只有副本可达 —— 我对 `xself::cmd_config` 的修改编进了二进制却完全不生效,查了三轮才发现。两份表已经漂移(`tui.interactive` vs `interactive`;`theme` 一个有条件一个无条件)。删掉副本。
2. **目录移动静默收窄了一个测试。** `test_windows_header_hygiene.py` 只扫 `ROOT/src`,而 `<windows.h>` 正好都在 `platform` 里。不修的话它会继续输出 "ok",检查的文件从 10 个变成 4 个 —— 而"找到的文件更少"和"违规更少"在输出上完全一样。这是本轮在治的形状,由本轮的移动亲手制造。
3. **包名在 mcpp 里是单一命名空间。** `modules/tinyhttps` 包装 `mcpplibs.tinyhttps`,同名导致"同一个名字既是版本依赖又是路径依赖"的解析错误。成员改名 `xhttp`。

**验证**:47 单测;E2E-61(13 场景)、E2E-48、subos-scope、shim、project、install-use、tui-output 全过;`--toolchain llvm@20.1.7` 全量重编(702 个 .o,零错误)复现 macOS/Windows 的 clang 语义。
