...
- [ ] ~~删除 P0.1 的 `includedir` 写入~~ —— **本次不删**（§0.15）：P3 的 reconciler
      推迟到 0.4.71，`cmd_use` 的头文件切换目前正是走这条路径，删掉会立刻重现 §3.1 的回归
...
| A18 | **降级安全** | 0.4.70 写出的状态回滚到 0.4.69 后，binding 关系仍有效 | #384 已双写成对边，P2 推迟后无格式变化，风险进一步降低 |
