...
**第一版判决（错的）**：一律按主线，弱证据不抢答，安装失败后再解释。
结果 aarch64 candidate 门禁直接红了 —— `--sandbox` 真的去下载了一个不存在的
bwrap：45 秒等待、一个 404、以及一句怪罪安装器而不点名目标的诊断。

**最终判决**：门槛取决于**是谁提的请求**。

| | 谁提的 | 判错的代价 | 弱证据够不够拒绝 |
|---|---|---|---|
| `xlings install <pkg>` | 用户点名 | 拒装一个本来能用的包，用户无从绕过 | ❌ 不够 |
| `--sandbox` 自动装后端 | 没人点名 | 少装一次，`xlings install bwrap` 仍可覆盖 | ✅ 够 |

这不是给主线开的口子，而是主线本来就缺的一维：**fail-closed 的门槛 = 证据强度
× 判错的代价**，而"用户有没有别的办法"正是代价的一部分。两半现在写在同一个
测试文件里（E2E-58），让这个不对称是**刻意的**，而不是两个互不知情的测试各自
长出来的。

无论哪一版，它都不再是被替换掉的 `#if defined(__aarch64__)` —— 那个把"索引里
有没有 aarch64 后端"这条会变的事实**冻进了二进制**；现在每次调用都重读磁盘上的
索引，零网络。
...
### 追加范围：issue #473（Windows）

用户要求在同一个 PR 里处理 [#473](https://github.com/openxlings/xlings/issues/473)。
根因落在本方案的 A1（silent-success）上，所以顺理成章：

- `create_shim` 调 `fs::remove(target)` 然后**丢弃错误**。Windows 删不掉正在运行的
  可执行文件，而 `self update` 要改写的 shim 恰恰就是当前进程的 `.exe`。后续的
  hardlink / copy 于是往一个仍被占用的路径写，报出用户看到的那句
  "the process cannot access the file because it is being used by another process"。
  新增 `platform::displace_locked_file`：Windows 用改名腾位（对运行中的映像有效），
  POSIX 就是 unlink。
- `ensure_subos_shims` 返回 `void`，把每一次 `create_shim` 的结果都扔了。这就是
  "healed 3" 的来源 —— **修复报告成功，是因为没人问过它成没成**。改为返回失败计数，
  三个调用点各自处理。

复现方式只有一种是忠实的：**用装好的二进制去重装它自己**，让被改写的 shim 是一个
