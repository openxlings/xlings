...
| 编号 | 决定 | 结论 |
|---|---|---|
| **D0** | 合并前先测 R0 | ✅ **实测确认是真回归，已修**。真实 home 上 ~25 个 SubOS 只有 3 个有 runtime 声明，其余全部会被升级盖成 `glibc@2.44` |
...
真实 home 上 `subos/*/.xlings.json` 的普查：**~25 个 SubOS，只有 3 个有 runtime 声明**
（`default`、`current`、`gfxbuild`，都是 `glibc@2.39`），其余全部 ABSENT。
构造后的对照：
