...
# Precondition. The fixture recipe writes a CONCRETE subos path, exactly as
# gcc.lua does today, and registration stores what the recipe wrote -- the
# core does not rewrite recipe values. So this is still the shape of the bug,
# and everything below asserts that execution is correct anyway and that
# doctor can migrate the record on request.
grep -q '/subos/dev' "$HOME_DIR/.xlings.json" \
  || fail "precondition lost: the fixture no longer bakes the install-time subos"
...
#
# Normalization is an exec-time read, not a migration, and registration
# stores what the recipe wrote. The recorded alias stays as installed --
# that is what makes this correct for a home whose subos selection differs
# per shell (XLINGS_ACTIVE_SUBOS) or per project, where no stored value
# could be right for all of them at once.
...
grep -q 'XLINGS_DYNAMIC_SUBOS_DIR' "$HOME_DIR/.xlings.json" \
...
log "A7: a recipe writing the marker needs no repair at all"
...
-- What a recipe SHOULD write: the marker, not an answer. The core expands it
-- at execution time and knows nothing about `--sysroot` itself.
local MARKER = "${XLINGS_DYNAMIC_SUBOS_DIR}"
function install()
    local bindir = path.join(pkginfo.install_dir(), "bin")
    os.tryrm(pkginfo.install_dir())
    os.mkdir(bindir)
...
# Stored verbatim -- the core does not rewrite recipe values -- and there is
# nothing to rewrite, because the recipe recorded a marker rather than one
# subos's answer to it.
grep -q 'XLINGS_DYNAMIC_SUBOS_DIR' "$HOME_DIR/.xlings.json" \
  || fail "A7: the marker was not stored verbatim"

out="$(RUN_IN default self doctor 2>&1 || true)"
if grep -q "sr-portable" <<<"$out"; then
  fail "A7: a record using the marker was reported as needing repair; got:\n$out"
fi
